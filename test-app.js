...
const feedback=boot();feedback.elements.languageToggle.onclick();feedback.run('game.collect({n:1});game.collect({n:3})');assert.match(feedback.elements.status.textContent,/碰到了黄色；本应收集橙色/);assert.equal(feedback.run('game.progress'),0);feedback.elements.languageToggle.onclick();assert.match(feedback.elements.status.textContent,/Expected orange/);feedback.run('selectLevel(BUILTINS[2])');assert.match(feedback.elements.status.textContent,/Take the detour/);console.log('PASS bilingual touched/expected feedback and teaching hint');
